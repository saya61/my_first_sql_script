-- 집계함수 상황예제 : 1, 2, 3 반 각각 30명씩 있을 때
-- '반 기준'으로 통계처리 SUM, AVG, COUNT
...
SELECT @_sql_mode;
SET sql_mode = '';    -- only group by 설정 날림
SET sql_mode = @_sql_mode;


-- NULL 처리? NULL 의 비중을 확인 후, 값이 왜곡되는 정도를 판단
-- eg) COUNT() 는 NULL 값 처리 X
-- but COUNT(*) 행의 수 카운트 하므로 NULL 행도 카운트
SELECT
    department_id,
    COUNT(id) AS employee_count,
    AVG(salary) AS average_salary
